...
const Dashboard = ({ isDark }) => {
  ...
  const downloadPDFReport = () => {
    ...
    // ── DARK HEADER BANNER ────────────────────────────────────────────────────
    drawRect(0, 0, pageW, 38, 53, 52, 53); // Deep Charcoal
    ...
    setFont('normal', 9);
    setColor(219, 212, 249); // Light Lilac
    ...
    const scoreColor = result.score >= 0.7 ? [255, 139, 203] : result.score >= 0.35 ? [196, 182, 206] : [167, 199, 255];
    ...
    drawRect(margin, y, contentW, 8, 163, 138, 178); // Electric Lavender
    setFont('bold', 10);
    setColor(255, 255, 255);
    doc.text('1. Inference Classification Diagnostics', margin + 3, y + 5.5);
    y += 14;

    const riskBg = result.score >= 0.7 ? [255, 240, 248] : result.score >= 0.35 ? [248, 245, 255] : [240, 248, 255];
    ...
    drawRect(margin, y, contentW, 8, 163, 138, 178);
    setFont('bold', 10);
    setColor(255, 255, 255);
    doc.text('2. Input Parameter Matrix Captured', margin + 3, y + 5.5);
    y += 12;

    drawRect(margin, y, contentW, 8, 196, 182, 206); // Soft Purple
    setFont('bold', 9);
    setColor(255, 255, 255);
    ...
    drawRect(margin, y, contentW, 8, 196, 182, 206);
    setFont('bold', 9);
    setColor(255, 255, 255);
  ...
  const getRiskTheme = () => {
    ...
    if (score >= 0.7) return { text: 'High Risk', color: isDark ? 'text-dm-neon-pink' : 'text-neon-pink', bg: isDark ? 'bg-dm-neon-pink/10' : 'bg-neon-pink/10', border: isDark ? 'border-dm-neon-pink/30' : 'border-neon-pink/30', glow: isDark ? 'shadow-[0_0_30px_rgba(236,72,153,0.3)]' : 'shadow-[0_0_30px_rgba(255,139,203,0.3)]', colorHex: isDark ? '#EC4899' : '#FF8BCB' };
    if (score >= 0.35) return { text: 'Medium Risk', color: isDark ? 'text-dm-neural-purple' : 'text-electric-lavender', bg: isDark ? 'bg-dm-neural-purple/10' : 'bg-electric-lavender/10', border: isDark ? 'border-dm-neural-purple/30' : 'border-electric-lavender/30', glow: isDark ? 'shadow-[0_0_30px_rgba(139,92,246,0.3)]' : 'shadow-[0_0_30px_rgba(163,138,178,0.3)]', colorHex: isDark ? '#8B5CF6' : '#A38AB2' };
    return { text: 'Low Risk', color: isDark ? 'text-dm-electric-blue' : 'text-cool-blue-glow', bg: isDark ? 'bg-dm-electric-blue/10' : 'bg-cool-blue-glow/10', border: isDark ? 'border-dm-electric-blue/30' : 'border-cool-blue-glow/30', glow: isDark ? 'shadow-[0_0_30px_rgba(59,130,246,0.3)]' : 'shadow-[0_0_30px_rgba(167,199,255,0.3)]', colorHex: isDark ? '#3B82F6' : '#A7C7FF' };
  };
  ...
  return (
    <div className="pt-32 pb-24 max-w-7xl mx-auto px-6 relative">
      ...
      <div className="text-center max-w-2xl mx-auto mb-12 space-y-3">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-electric-lavender/10 dark:bg-dm-electric-blue/10 border border-electric-lavender/30 dark:border-dm-border text-electric-lavender dark:text-dm-electric-blue text-xs font-bold uppercase tracking-wider">
          <HeartPulse className="w-3.5 h-3.5 animate-pulse" /> Neural Forecast Engine
        </div>
        <h1 className="text-4xl lg:text-5xl font-extrabold font-outfit text-primary-heading dark:text-dm-text-primary">Clinical Diagnostics</h1>
        <p className="text-secondary-text dark:text-dm-text-secondary">Input patient parameters below to evaluate cognitive risk boundaries and generate a downloadable report.</p>
      </div>
      ...
      <AnimatePresence>
        {error && (
          <motion.div 
            ...
            className="bg-neon-pink/10 dark:bg-dm-neon-pink/10 border border-neon-pink/30 dark:border-dm-neon-pink/30 text-primary-heading dark:text-dm-text-primary p-4 rounded-2xl flex items-center gap-3 mb-8 shadow-[0_0_20px_rgba(255,139,203,0.15)] dark:shadow-[0_0_20px_rgba(236,72,153,0.15)]"
          >
            <AlertTriangle className="w-5 h-5 text-neon-pink dark:text-dm-neon-pink shrink-0" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="glass-panel p-8 relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-electric-lavender/40 dark:via-dm-electric-blue/40 to-transparent" />
            ...
            <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
              <h2 className="text-xl font-bold font-outfit text-primary-heading dark:text-dm-text-primary flex items-center gap-2">
                <Stethoscope className="text-electric-lavender dark:text-dm-electric-blue w-5 h-5" /> Patient Vitals
              </h2>
              <div className="flex gap-2.5">
                <button type="button" onClick={() => loadPreset('healthy')}
                  className="px-3.5 py-1.5 rounded-full border border-cool-blue-glow/40 dark:border-dm-electric-blue/40 bg-white/60 dark:bg-black/20 text-xs text-cool-blue-glow dark:text-dm-electric-blue font-semibold hover:bg-white/80 dark:hover:bg-black/40 transition-all flex items-center gap-1 shadow-sm">
                  <UserCheck className="w-3.5 h-3.5" /> Healthy Demo
                </button>
                <button type="button" onClick={() => loadPreset('risk')}
                  className="px-3.5 py-1.5 rounded-full border border-neon-pink/40 dark:border-dm-neon-pink/40 bg-white/60 dark:bg-black/20 text-xs text-neon-pink dark:text-dm-neon-pink font-semibold hover:bg-white/80 dark:hover:bg-black/40 transition-all flex items-center gap-1 shadow-sm">
            ...
            <form onSubmit={triggerAnalysis} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={field.name} className="relative">
                    <input type={field.type} name={field.name} placeholder=" " value={formData[field.name]} onChange={handleInputChange}
                      className="floating-input w-full bg-white/50 dark:bg-[#0B1020]/40 border border-white/80 dark:border-dm-border rounded-xl px-4 py-3.5 focus:outline-none focus:border-electric-lavender dark:focus:border-dm-electric-blue focus:shadow-[0_0_15px_rgba(163,138,178,0.2)] dark:focus:shadow-[0_0_15px_rgba(59,130,246,0.15)] text-primary-heading dark:text-dm-text-primary transition-all duration-300 shadow-inner dark:shadow-none" />
                ...
                <div className="relative">
                  <select name="gender" value={formData.gender} onChange={handleInputChange}
                    className="w-full bg-white/50 dark:bg-[#0B1020]/40 border border-white/80 dark:border-dm-border rounded-xl px-4 py-3.5 focus:outline-none focus:border-electric-lavender dark:focus:border-dm-electric-blue text-primary-heading dark:text-dm-text-primary transition-all duration-300 text-sm shadow-inner dark:shadow-none">
                  ...
                  <span className="absolute -top-2.5 left-4 bg-white/90 dark:bg-dm-bg-navy backdrop-blur px-2 rounded text-xs text-electric-lavender dark:text-dm-electric-blue font-semibold">Gender</span>
                </div>
              ...
              <div className="flex gap-4 pt-4">
                <button type="submit" disabled={analyzing}
                  ...
                  <span className="relative z-10 text-white-text">{analyzing ? 'Analyzing Vitals...' : 'Start AI Prediction'}</span>
                ...
                <button type="button" onClick={resetForm}
                  className="px-6 py-4 rounded-xl border border-white/80 dark:border-dm-border bg-white/50 dark:bg-black/20 hover:bg-white/80 dark:hover:bg-black/40 transition-all font-bold text-dark-gray-text dark:text-dm-text-secondary flex items-center gap-2 shadow-sm">
        ...
        <div className="lg:col-span-5 h-full">
          <div className="glass-panel p-8 relative overflow-hidden h-full flex flex-col justify-between min-h-[500px]">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-soft-purple/40 dark:via-dm-neural-purple/40 to-transparent" />
            
            <AnimatePresence mode="wait">
              {!result && !analyzing && (
                <motion.div key="waiting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="flex flex-col items-center justify-center text-center py-16 h-full flex-grow space-y-6">
                  <div className="w-20 h-20 bg-white/50 dark:bg-black/20 rounded-full border border-white/80 dark:border-dm-border flex items-center justify-center shadow-inner">
                    <BrainCircuit className="text-electric-lavender/50 dark:text-dm-electric-blue/50 w-10 h-10 animate-pulse" />
                  </div>
                  <h3 className="text-xl font-bold font-outfit text-primary-heading dark:text-dm-text-primary opacity-70">Awaiting Vitals</h3>
                  <p className="text-secondary-text dark:text-dm-text-secondary text-sm max-w-xs leading-relaxed">Select a sample demo preset or manually input vitals to trigger deep neural inference.</p>
                </motion.div>
              )}
              ...
              {analyzing && (
                <motion.div key="analyzing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div className="relative">
                    <div className="w-36 h-36 border-2 border-electric-lavender/40 dark:border-dm-electric-blue/40 rounded-full animate-ping" />
                    <div className="absolute inset-0 w-36 h-36 border-2 border-t-electric-lavender dark:border-t-dm-electric-blue border-transparent rounded-full animate-spin" style={{ animationDuration: '1s' }} />
                    <div className="absolute inset-0 m-auto w-28 h-28 bg-white/60 dark:bg-dm-bg-section border border-white/90 dark:border-dm-border rounded-full flex items-center justify-center shadow-lg">
                      <BrainCircuit className="text-electric-lavender dark:text-dm-electric-blue w-12 h-12 floating" />
                    </div>
                  </div>
                  <div className="space-y-3 w-full max-w-xs">
                    <h3 className="text-xl font-bold font-outfit text-primary-heading dark:text-dm-text-primary">Synaptic Screening</h3>
                    <div className="space-y-1 bg-white/50 dark:bg-black/20 border border-white/80 dark:border-dm-border p-3.5 rounded-xl text-left text-xs font-mono text-secondary-text dark:text-dm-text-muted leading-relaxed shadow-inner dark:shadow-none">
                      <p className={loadingStep >= 0 ? 'text-electric-lavender dark:text-dm-electric-blue font-semibold' : ''}>{loadingStep >= 0 ? '✓' : '○'} Preprocessing clinical vitals...</p>
                      <p className={loadingStep >= 1 ? 'text-soft-purple dark:text-dm-neural-purple font-semibold' : ''}>{loadingStep >= 1 ? '✓' : '○'} Simulating cognitive signal wave...</p>
                      <p className={loadingStep >= 2 ? 'text-neon-pink dark:text-dm-neon-pink font-semibold' : ''}>{loadingStep >= 2 ? '✓' : '○'} Querying advanced Keras model...</p>
                      <p className={loadingStep >= 3 ? 'text-cool-blue-glow dark:text-dm-cyan-glow font-semibold animate-pulse' : ''}>{loadingStep >= 3 ? '✓' : '○'} Resolving risk interpretations...</p>
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {result && !analyzing && (
                <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}
                  ...
                  <div className={`p-6 rounded-2xl border ${riskTheme.border} ${riskTheme.bg} bg-white/50 dark:bg-transparent relative overflow-hidden flex items-center justify-between flex-wrap gap-6 shadow-sm`}>
                    ...
                    <div className="space-y-2">
                      ...
                      <h3 className="text-xl font-extrabold font-outfit text-primary-heading dark:text-dm-text-primary leading-tight">Cognitive Screening Complete</h3>
                      <p className="text-xs text-secondary-text dark:text-dm-text-muted">Diagnostic code: <span className="font-mono text-electric-lavender dark:text-dm-electric-blue">AI-NEURO-{scorePercent}</span></p>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <h4 className="text-xs font-extrabold text-secondary-text dark:text-dm-text-muted uppercase tracking-widest flex items-center gap-1.5">
                      <Sparkles className="w-4 h-4 text-electric-lavender dark:text-dm-electric-blue" /> AI Interpretation
                    </h4>
                    <p className="text-sm text-primary-heading dark:text-dm-text-primary leading-relaxed bg-white/60 dark:bg-black/20 border border-white/90 dark:border-dm-border p-4 rounded-xl shadow-inner dark:shadow-none">{result.interpretation}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/60 dark:bg-black/20 border border-white/90 dark:border-dm-border px-3 py-2 rounded-xl text-left shadow-sm">
                      <p className="text-[10px] text-secondary-text dark:text-dm-text-muted uppercase font-semibold">Hypertensive State</p>
                      <p className="text-xs font-bold text-primary-heading dark:text-dm-text-primary mt-0.5">
                        {parseInt(formData.systolicBP) >= 140 || parseInt(formData.diastolicBP) >= 90 ? <span className="text-neon-pink dark:text-dm-neon-pink">Severe Hypertensive</span>
                          : parseInt(formData.systolicBP) >= 120 ? <span className="text-soft-purple dark:text-dm-neural-purple">Elevated State</span>
                          : <span className="text-electric-lavender dark:text-dm-electric-blue">Optimal Range</span>}
                      </p>
                    </div>
                    <div className="bg-white/60 dark:bg-black/20 border border-white/90 dark:border-dm-border px-3 py-2 rounded-xl text-left shadow-sm">
                      <p className="text-[10px] text-secondary-text dark:text-dm-text-muted uppercase font-semibold">Inflammatory Check</p>
                      <p className="text-xs font-bold text-primary-heading dark:text-dm-text-primary mt-0.5">
                        {parseInt(formData.wbcCount) > 11000 ? <span className="text-neon-pink dark:text-dm-neon-pink">Elevated WBC Count</span>
                          : parseInt(formData.wbcCount) < 4000 ? <span className="text-soft-purple dark:text-dm-neural-purple">Mild Leukopenia</span>
                          : <span className="text-electric-lavender dark:text-dm-electric-blue">Normal Bio-count</span>}
                      </p>
                    </div>
                  </div>
